...
- Audience & language: write for framework **users**, not for the author or next maintainer. The headline and the first sentence of the blockquote answer "what can I now do, or what no longer breaks?" in plain terms. **Internal type names, helper utilities, and inference mechanics (`TReq`, `Checked<TContract, TReq>`, `AnyReqAuthorizeConfig`, contravariance, conditional-type tricks) do NOT appear in headlines, blockquotes, or bullets** — they live in the linked spec. Bullets describe **observable behavior**, not the mechanism implementing it. A user reading the headline alone must know whether the change affects them. This is a recurring failure mode — see the BAD-vs-GOOD contrast example in `docs/specs/2026-07-17-changelog.md` and verify your draft against it before finishing. Jargon is not precision; it is a readability regression.
